...
class Stack:    
    ...
    def push(self,E):
        if(self.is_full()):
            ...
            self.__top+=1
            self._lis_of_eles.insert(self._top,E)
            #print("Pushed ele: ",self._lis_of_eles[self._top])
    ...
    def pop(self):
        if(self.is_empty()):
            ...
            del_ele=self._lis_of_eles.pop(self._top)
...
class Equation:
    ...
    def validate(self):
        ...
        if((f not in Equation.operands_lower_case and f not in Equation.operands_upper_case) or
           (self._last_symbol not in Equation.operands_lower_case and self._last_symbol not in Equation.operands_upper_case)):
